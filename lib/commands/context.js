...
extensions.listWebFrames = async function (useUrl = true) {
  ...
  /* jshint ignore:start */
  logger.debug(`Selecting by url: ${useUrl} ${useUrl ? `(expected url: '${currentUrl}')` : ''}`);
  /* jshint ignore:end */
  ...
  if (this.isRealDevice() && this.remote && this.opts.bundleId) {
    ...
    logger.info('CREATING NEW REMOTE DEBUGGER');
    logger.info(this.opts.bundleId, this.useNewSafari(), this.pageLoadMs, this.opts.platformVersion);
    ...
    logger.info('INSTANCE CREATED', currentUrl);
    let appInfo = await this.remote.connect();
    if (!appInfo) {
      logger.debug('Unable to connect to the remote debugger.');
      return [];
    }
    logger.info('URL BEING USED:', currentUrl);

    await B.delay(3000);
    logger.info('paused. now continueing');
