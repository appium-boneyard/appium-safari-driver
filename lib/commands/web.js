...
commands.title = async function () {
  ...
  logger.debug('getting title using executeAtom.....', this.executeAtom);
  let title = await this.executeAtom('title', [], true);
...
helpers.executeAtom = async function (atom, args, alwaysDefaultFrame = false) {
  ...
  logger.debug('in executeAtom, about to wait for atom. did we get promise?', promise);
  promise.then(logger.warn, logger.error);
